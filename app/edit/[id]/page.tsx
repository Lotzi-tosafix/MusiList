...
function SortableVideoItem({ video, index }: { video: any, index: number }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-4 bg-slate-800 p-3 rounded-xl border border-slate-700">
...
export default function EditPlaylist({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      setVideos((items) => {
        const oldIndex = items.findIndex(item => item.youtube_id === active.id);
        const newIndex = items.findIndex(item => item.youtube_id === over?.id);
  ...
  const toggleDateSort = () => {
    ...
    const sorted = [...videos].sort((a, b) => {
      const timeA = a.published_at ? new Date(a.published_at).getTime() : (a.publishedAt ? new Date(a.publishedAt).getTime() : 0);
      const timeB = b.published_at ? new Date(b.published_at).getTime() : (b.publishedAt ? new Date(b.publishedAt).getTime() : 0);
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 space-y-3">
              <label className="block text-sm font-medium text-slate-300">הגדרות פרטיות</label>
              <div className="flex gap-4">
                <button
                  type="button"
                  onClick={() => setIsPublic(true)}
                  className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border transition-all ${isPublic ? 'bg-violet-600/20 border-violet-500 text-violet-300' : 'bg-slate-900 border-slate-700 text-slate-400 hover:bg-slate-800'}`}
                >
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                {(alphaSortDir || dateSortDir || viewsSortDir || videos.some((v, i) => v.youtube_id !== originalVideos[i]?.youtube_id)) && (
                  <button onClick={resetOrder} className="flex items-center justify-center px-3 py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 transition-colors border border-red-500/30" title="אפס סדר">
                    <RotateCcw className="w-4 h-4" />
                  </button>
                )}
                <button onClick={toggleAlphaSort} className={`flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg transition-colors border ${alphaSortDir ? 'bg-violet-900/40 border-violet-500/50 text-violet-300' : 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-300'}`}>
                  {alphaSortDir === 'desc' ? <ArrowUpAZ className="w-4 h-4" /> : <ArrowDownAZ className="w-4 h-4" />}
                  א-ב
                </button>
                <button onClick={toggleDateSort} className={`flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg transition-colors border ${dateSortDir ? 'bg-violet-900/40 border-violet-500/50 text-violet-300' : 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-300'}`}>
                  <CalendarDays className="w-4 h-4" /> תאריך
                  {dateSortDir === 'asc' ? <ArrowUp className="w-3 h-3" /> : <ArrowDown className="w-3 h-3" />}
                </button>
                <button onClick={toggleViewsSort} className={`flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg transition-colors border ${viewsSortDir ? 'bg-violet-900/40 border-violet-500/50 text-violet-300' : 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-300'}`}>
            ...
            <p className="text-sm text-slate-400">גרור סרטונים כדי לשנות את סדר ההשמעה שלהם בפלייליסט.</p>
              ...
              >
                ...
                >
                  {videos.map((video, index) => (
                    <SortableVideoItem key={video.youtube_id} video={video} index={index} />
                  ))}
                </SortableContext>
              </DndContext>
            </div>
